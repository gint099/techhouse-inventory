        ...
        <div class="container-fluid">
            ...
            <div class="card card-success card-outline shadow-sm">
                <div class="card-header">
                    <h3 class="card-title"><i class="bi bi-box-arrow-in-down me-2"></i>Transaksi Barang Masuk</h3>
                    <div class="card-tools">
                        <a href="{{ route('item-ins.create') }}" class="btn btn-success btn-sm">
                ...
                <div class="card-body">
                    <form method="GET" action="{{ route('item-ins.index') }}" class="row mb-3">
                        <div class="col-md-3">
                        ...
                        <div class="col-md-3">
                            <input type="date" name="tanggal_masuk" value="{{ $date }}" class="form-control">
                        </div>
                        <div class="col-md-2">
                            ...
                            <tbody>
                                @forelse ($itemIns as $itemIn)
                                    <tr>
                                        ...
                                        <td><span class="badge bg-dark">{{ $itemIn->kode_transaksi }}</span></td>
                                        <td>{{ $itemIn->tanggal_masuk->format('d M Y') }}</td>
                                        <td>{{ $itemIn->item->name }}</td>
                                        <td>{{ $itemIn->quantity }}</td>
                                        <td>{{ $itemIn->user->name }}</td>
                                        <td>
                                            <a href="{{ route('item-ins.show', $itemIn) }}" class="btn btn-info btn-sm"><i class="bi bi-eye"></i></a>
                                            <form action="{{ route('item-ins.destroy', $itemIn) }}" method="POST" class="d-inline" onsubmit="return confirm('Hapus transaksi ini? Stok akan dikurangi.')">
                                                @csrf
                                                @method('DELETE')
